...
namespace Anzuelo.Infraestructure.Repository.Implementations
{
    public class RepositoryEstadoPedido : IRepositoryEstadoPedido
    {
        ...
        {
            var collection =
                await _context.Set<EstadoPedido>()
        ...
        public async Task<EstadoPedido> FindByIdAsync(int id)
        {
            var @object =
                await _context.Set<EstadoPedido>()
                    .FirstOrDefaultAsync(
                        x => x.IdEstadoPedido == id);

            return @object!;
        }

        public async Task<EstadoPedido?>
            FindByDescripcionAsync(
                string descripcion)
        {
            var texto =
                descripcion.Trim().ToLower();
